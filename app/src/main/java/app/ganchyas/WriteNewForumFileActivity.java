...
/**
 * Allows the user to upload a forum with an file
 * @author Paradox
 */
public class WriteNewForumFileActivity extends AppCompatActivity {
    ...
    EditText mainText;
    /**
     * Reference to the edit text on xml that the user enters the file name into
     */
    EditText fileName;
    /**
    ...
    /**
     * Uri of the file needed to be uploaded
     */
    Uri fileUri;
    /**
    ...
    /**
     * Contains the file name of the forum
     */
    ...
    /**
     * Overriding onCreate to Inflate custom UI using activity_write_new_forum_file.xml
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_write_new_forum_file);

        dialog = new ProgressDialog(WriteNewForumFileActivity.this);
        ...
        fileName = findViewById(R.id.forumFileName);
    }
    ...
    public void submitForumAction(View view) {
        ...
        if (fileUri != null) {
            if (!fileNameValue.equals("") && !fileNameValue.equals(" ")) {
                dialog.setMessage("Uploading the file ....");
                dialog.show();
                uploadFile();
            }
            else{
                CommonMethods.toastMessage(WriteNewForumFileActivity.this, "File name is required");
            }
        }

        else {
            CommonMethods.toastMessage(WriteNewForumFileActivity.this, "File is required");
        }
    }
    ...
    private void checkFilePermissions() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            permissionCheck = WriteNewForumFileActivity.this.checkSelfPermission("Manifest.permission.READ_EXTERNAL_STORAGE");
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            permissionCheck += WriteNewForumFileActivity.this.checkSelfPermission("Manifest.permission.WRITE_EXTERNAL_STORAGE");
        }
    ...
    public void uploadFile() {
        ...
        final String extension = getFileExtension(fileUri);
        final StorageReference fileRef = completeStorageReference.child("forum files/" + identifier + "/" + fileNameValue + "." + extension);
        fileRef.putFile(fileUri).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
            @Override
            public Task<Uri> then(@NonNull Task<UploadTask.TaskSnapshot> task) throws Exception {
                if (!task.isSuccessful()) {
                    dialog.dismiss();
                    Toast.makeText(WriteNewForumFileActivity.this, "Upload Failed", Toast.LENGTH_SHORT).show();
                }
                ...
                return fileRef.getDownloadUrl();
                                     ...
                                     public void onComplete(@NonNull Task<Uri> task) {
                                         ...
                                         values.put("fileName", fileNameValue + "." + extension);
                                         ...
                                         dialog.dismiss();
                                         Intent intent = new Intent(WriteNewForumFileActivity.this, MainActivity.class);
            ...
            public void onFailure(@NonNull Exception e) {
                CommonMethods.toastMessage(WriteNewForumFileActivity.this, "uploading failed");
            }
        });
